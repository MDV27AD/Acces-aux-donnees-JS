...
export const createProduct = async (req, res) => {
    const { sku: p_sku, name: p_name, description: p_description, updatedAt: p_last_update, seller : { id
        , name, created_at: creation_date}, price: p_price, serialNumber: p_serial_number, category } = req.body
...
export const getAvailableProducts = async (_req, res) => {
    const menus = await Product.find({ p_status: 'En stock' })
    res.status(200).json(menus)
}